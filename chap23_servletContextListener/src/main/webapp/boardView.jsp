...
<title>게시물 내용보기</title>
	...
	<script type="text/javascript">
		...
		// 수정 체크
		function modifyCheck(url) {
			var id = '${sessionScope.member.id}';
			if(id == '${board.id}'){
				location.href = url;
			}else{
				alert('해당 글 작성자만 글을 수정할 수 있습니다.');
			}
		}
		// 답글 체크
		function replyCheck(url) {
			var id = '${sessionScope.member.id}';
			if(id != null && id.length > 0){
				location.href = url;
			}else{
				alert('로그인 사용자만 글을 답글할 수 있습니다.');
...
<body>
	...
	<div>
		...
		<p class="btn_align">
			...
			<input type="button" value="수정" onclick="modifyCheck('<c:url value="/boardModify?no=${board.no}" />');" />
			<input type="button" value="삭제" onclick="deleteCheck('<c:url value="/boardDelete?no=${board.no}" />');" />
			<input type="button" value="답글쓰기" onclick="replyCheck('<c:url value="/boardReplyWrite?no=${board.no}" />');" />
		</p>
	</div>	
</body>
</html>