...
public class BoardDao {
	...
	public List<BoardVo> selectList(int pageNum) {
		 ...
		 * 끝번호 : 시작번호 + 한 페이지 보여줄 수 - 1
		 * 한 페이지에 보여줄 게시물 수는 10개로 가정
		 */
		start = (pageNum - 1) * 10 + 1;
		end = start + 10 - 1;
		
		// [디버깅]
		System.out.println("시작게시물번호start : " + start + " / 끝게시물번호 : " + end);

		// [2. 방법] row_number()함수 사용 - 게시물 번호로 역순 정렬
		
		query.append("select a.no, a.title, a.content, a.id, a.hit, a.regdate,")
			 .append(" a.reply_group, a.reply_order, a.reply_indent ")
			 .append(" from( ")
			 .append(" 	select b.*, row_number() over(order by b.no asc) row_num")
