...
public class LoginServelet2 extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (request.getParameter("errorMsg") != null) {
			...
			/*
			 * [디코딩]
			 * 오류 메세지 보내는 쪽에서 utf-8 방식으로 2byte씩 인코딩 했으므로
			 * 받는쪽에서도 utf-8 방식으로 2byte씩 디코딩해야 완벽한 메세지 복원됨.
			 */
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (isAuthenticated) {
			...
			/*
			 * [한글 메시지의 경우 인코딩]
			 * - 한글 메시지를 Url에 쿼리스트링 형태로 달고 가기 위해 인코딩
			 ...
			 *   두 자리 16진수 숫자로 대체됨.
			 * - 인코딩 방식 : 입력된 문자를 ascii 테이블에서 매칭되는 hex 값에
			 *   %를 앞에 붙이면 됨. (퍼센트 인코딩)
			...
			/*
			 * sendRedirect는 get방식 요청으로 주소창에 파라미터를 달고 감.
			 */
